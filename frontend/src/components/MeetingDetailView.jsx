...
export default function MeetingDetailView({ meetingId, onBack, onRefresh }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const r = await api.get(`/api/meetings/${meetingId}`)
      const m = r.data.meeting || r.data
      setMeeting(m)
      setMeetingNotes(m.meeting_notes || '')
  ...
  const loadDocs = async () => {
    try { const r = await api.get(`/api/meetings/${meetingId}/documents`); setDocs(r.data.documents) }
    catch(e) {}
  }

  const loadActivities = async () => {
    try { const r = await api.get(`/api/meetings/${meetingId}/activities`); setActivities(r.data.activities) }
    catch(e) {}
  }

  const loadShares = async () => {
    try { const r = await api.get(`/api/meetings/${meetingId}/share`); setShares(r.data.shares) }
    catch(e) {}
  }

  const loadUsers = async () => {
    try {
      const r = await api.get('/api/users?role=user,admin')
  ...
  const saveNotes = async () => {
    setSavingNotes(true)
    try {
      const r = await api.put(`/api/meetings/${meetingId}`, { meeting_notes: meetingNotes })
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await api.post(`/api/meetings/${meetingId}/documents`, fd)
  ...
  const deleteDoc = async (doc) => {
    ...
    try {
      await api.delete(`/api/meetings/${meetingId}/documents/${doc.id}`)
      loadDocs(); loadActivities()
  ...
  const updateStatus = async (status) => {
    try {
      const r = await api.put(`/api/meetings/${meetingId}`, { status })
      setMeeting(r.data.meeting)
      loadActivities()
  ...
  const shareWithUsers = async () => {
    ...
    try {
      await api.post(`/api/meetings/${meetingId}/share`, { user_ids: selectedUsers })
  ...
  const removeShare = async (uid) => {
    try {
      await api.delete(`/api/meetings/${meetingId}/share/${uid}`)
      loadShares(); loadActivities()
  ...
  if (!meeting) return <p style={{ textAlign: 'center', padding: 40, color: '#94A3B8' }}>Meeting not found</p>

  const sStyle = STATUS_STYLES[meeting.status] || { bg: '#F3F4F6', text: '#6B7280' }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div style={{ padding: '14px', background: '#F8FAFC', borderRadius: '10px' }}>
              <p style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 4px' }}>Date & Time</p>
              <p style={{ fontSize: 14, fontWeight: 500, color: '#1F2937', margin: 0 }}>{formatDT(meeting.meeting_date)}</p>
            </div>
            <div style={{ padding: '14px', background: '#F8FAFC', borderRadius: '10px' }}>
              <p style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 4px' }}>Status</p>
              <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: '6px', fontSize: 12, fontWeight: 600, background: sStyle.bg, color: sStyle.text }}>{meeting.status}</span>
            </div>
            <div style={{ padding: '14px', background: '#F8FAFC', borderRadius: '10px' }}>
              <p style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 4px' }}>Created By</p>
              <p style={{ fontSize: 14, fontWeight: 500, color: '#1F2937', margin: 0 }}>{meeting.created_by_name || '—'}</p>
            </div>
            <div style={{ padding: '14px', background: '#F8FAFC', borderRadius: '10px' }}>
              <p style={{ fontSize: 11, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 4px' }}>Location</p>
              <p style={{ fontSize: 14, fontWeight: 500, color: '#1F2937', margin: 0 }}>{meeting.location || '—'}</p>
            </div>
          </div>
          ...
          {meeting.meeting_link && (
            <div style={{ padding: '14px', background: '#EEF2FF', border: '1px solid #C7D2FE', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <a href={meeting.meeting_link} target="_blank" rel="noopener noreferrer" style={{ color: '#4338CA', fontWeight: 500, textDecoration: 'none', fontSize: 13, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{meeting.meeting_link}</a>
            </div>
          )}
          ...
          {meeting.description && (
            <div style={{ padding: '14px', background: '#F8FAFC', borderRadius: '10px' }}>
              ...
              <p style={{ fontSize: 13, color: '#1F2937', margin: 0, whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>{meeting.description}</p>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {meeting.meeting_link && (
              <a href={meeting.meeting_link} target="_blank" rel="noopener noreferrer" style={{ padding: '10px 22px', borderRadius: '8px', border: 'none', background: '#059669', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
      ...
      case 'notes': return (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            ...
            {meeting.meeting_notes && !editingNotes && (
              <button onClick={() => { setEditingNotes(true); setMeetingNotes(meeting.meeting_notes) }} style={{ background: 'none', border: 'none', color: '#5B3DF5', cursor: 'pointer', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                <Edit3 className="w-3.5 h-3.5" /> Edit
              </button>
            )}
          </div>
          {editingNotes || !meeting.meeting_notes ? (
            <>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                ...
                {meeting.meeting_notes && (
                  <button onClick={() => { setEditingNotes(false); setMeetingNotes(meeting.meeting_notes) }} style={{ padding: '10px 24px', borderRadius: '8px', border: '1px solid #D1D5DB', background: '#fff', color: '#6B7280', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                    Cancel
                  </button>
                )}
              </div>
            ...
            <div style={{ padding: '20px', background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: '12px' }}>
              <p style={{ fontSize: 14, color: '#92400E', margin: 0, whiteSpace: 'pre-wrap', lineHeight: 1.8 }}>{meeting.meeting_notes}</p>
            </div>
          )}
        </div>
      )
                ...
                return (
                  <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      <a href={`/api/meetings/${meetingId}/documents/${d.id}`} target="_blank" rel="noopener noreferrer" style={{ padding: '6px', borderRadius: '6px', color: '#6B7280', textDecoration: 'none' }}>
            ...
            <div style={{ position: 'relative' }}>
              {activities.map((a, i) => (
                <div key={a.id} style={{ display: 'flex', gap: '14px', paddingBottom: i < activities.length - 1 ? '20px' : 0, position: 'relative' }}>
                  ...
                  {/* Content */}
                  <div style={{ flex: 1, paddingBottom: i < activities.length - 1 ? 0 : 0 }}>
          ...
          {!showShareForm ? (
            ...
            <div style={{ marginBottom: '16px', padding: '16px', background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '10px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '200px', overflowY: 'auto', marginBottom: '12px' }}>
                {allUsers.filter(u => u.id !== meeting.created_by).map(u => (
    ...
    <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #E2E8F0', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E8F0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0F172A', margin: 0 }}>{meeting.title}</h2>
            <p style={{ fontSize: 13, color: '#64748B', margin: '2px 0 0' }}>{formatDT(meeting.meeting_date)}</p>
          </div>
          <span style={{ marginLeft: 'auto', padding: '4px 12px', borderRadius: '6px', fontSize: 11, fontWeight: 600, background: sStyle.bg, color: sStyle.text, whiteSpace: 'nowrap' }}>{meeting.status}</span>
        </div>
      </div>
