...
export default function TaskRow({ task, onStatusToggle, onAddSubtask, team, addSubtaskOf, setAddSubtaskOf, subtaskForm, setSubtaskForm, onAddSubtaskSubmit, onTaskClick }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid #F3F4F6', padding: '12px 0', background: task.status === 'Completed' ? '#FAFAFA' : 'transparent' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span onClick={() => onTaskClick?.(task)} style={{ fontSize: 15, fontWeight: 600, color: task.status === 'Completed' ? C.muted : '#1F2937', flex: 1, textDecoration: task.status === 'Completed' ? 'line-through' : 'none', cursor: onTaskClick ? 'pointer' : 'default' }}>
          {task.title}
        </span>
        <span style={{ fontSize: 13, color: task.assigned_name ? '#6B7280' : '#9CA3AF', whiteSpace: 'nowrap' }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 12px', borderRadius: 20, background: badge.bg, color: badge.color, whiteSpace: 'nowrap' }}>
          {task.status}
        </span>
        <button onClick={() => { setAddSubtaskOf(task.id); setSubtaskForm({ title: '', assigned_to: '', due_date: '', status: 'Open' }) }}
          style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid #D1D5DB', background: '#fff', color: '#6B7280', fontSize: 12, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
          + Subtask
        </button>
      </div>
      {task.subtasks?.map(st => (
        <div key={st.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0 4px 32px' }}>
          ...
          <span style={{ fontSize: 12, color: '#9CA3AF' }}>{st.assigned_name || ''}</span>
          <span style={{ fontSize: 12, color: '#9CA3AF' }}>{st.due_date ? new Date(st.due_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) : ''}</span>
          <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 10px', borderRadius: 20, background: st.status === 'Completed' ? '#D1FAE5' : '#F3F4F6', color: st.status === 'Completed' ? '#065F46' : '#6B7280' }}>{st.status}</span>
        </div>
      ))}
      {addSubtaskOf === task.id && (
        <form onSubmit={onAddSubtaskSubmit} style={{ display: 'flex', gap: 10, padding: '10px 0 6px 32px', flexWrap: 'wrap' }}>
          ...
          <select value={subtaskForm.assigned_to} onChange={e => setSubtaskForm({ ...subtaskForm, assigned_to: e.target.value })}
            style={{ padding: '8px 12px', border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, background: '#fff', fontFamily: 'inherit' }}>
          ...
          <button type="submit" style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: C.primary, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Add</button>
          <button type="button" onClick={() => setAddSubtaskOf(null)} style={{ padding: '8px 16px', borderRadius: 8, border: `1.5px solid ${C.border}`, background: '#fff', color: '#6B7280', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
        </form>
      )}
    </div>
  )
