...
export default function PhaseCard({ phase, index, team, addTaskPhase, setAddTaskPhase, phaseTaskForm, setPhaseTaskForm, addSubtaskOf, setAddSubtaskOf, subtaskForm, setSubtaskForm, onAddTaskToPhase, onAddSubtaskSubmit, onTaskStatusToggle, onTaskClick }) {
  ...
  return (
      <div style={{ marginBottom: 20, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '18px 22px', background: '#F8FAFC', borderBottom: `1px solid ${C.border}` }}>
          <div style={{ width: 32, height: 32, borderRadius: '50%', background: C.primaryLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: C.primary, flexShrink: 0 }}>{index + 1}</div>
          <span style={{ fontSize: 17, fontWeight: 700, color: '#1F2937', flex: 1 }}>{phase.name}</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 80, height: 8, borderRadius: 4, background: '#E5E7EB', overflow: 'hidden' }}>
              <div style={{ width: `${pct}%`, height: '100%', borderRadius: 4, background: pct === 100 ? '#059669' : C.primary, transition: 'width 0.3s' }} />
            </div>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.secondary, whiteSpace: 'nowrap' }}>{completed}/{total}</span>
          </div>
        ...
        <div style={{ padding: '8px 22px 14px' }}>
          ...
          {(!phase.tasks || phase.tasks.length === 0) && (
            <div style={{ fontSize: 14, color: C.muted, padding: '16px 0', textAlign: 'center' }}>No tasks in this phase</div>
          )}
        </div>
          ...
          {addTaskPhase === phase.id ? (
            <form onSubmit={onAddTaskToPhase} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <input value={phaseTaskForm.title} onChange={e => setPhaseTaskForm({ ...phaseTaskForm, title: e.target.value })} placeholder="Task title" required
                style={{ flex: '1 1 200px', padding: '8px 12px', border: `1.5px solid ${C.border}`, borderRadius: 8, fontSize: 13, outline: 'none', fontFamily: 'inherit' }} />
              ...
              <button type="button" onClick={() => setAddTaskPhase(null)} style={{ padding: '8px 16px', borderRadius: 8, border: `1.5px solid ${C.border}`, background: '#fff', color: '#6B7280', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
            </form>
          ) : (
            <button onClick={() => { setAddTaskPhase(phase.id); setPhaseTaskForm({ title: '', assigned_to: '', due_date: '', status: 'Open' }) }}
              style={{ padding: '8px 18px', borderRadius: 8, border: '1.5px dashed #D1D5DB', background: 'none', color: C.primary, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              + Add Task
            </button>
          )}
        </div>
