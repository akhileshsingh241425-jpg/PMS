...
const nav = [
  ...
  { to: '/accounts', icon: Building2, label: 'Clients' },
...
function NotifBell() {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={toggle} className="relative p-2 rounded-xl hover:bg-slate-100 text-slate-400 transition-colors">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl border border-slate-200 z-50 max-h-96 overflow-y-auto shadow-lg">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            ...
            {count > 0 && <button onClick={markAllRead} className="text-xs text-[#6D28D9] font-semibold hover:underline">Mark all read</button>}
          </div>
          {notifs.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-10">No new notifications</p>
          ) : (
            notifs.map(n => (
              <div key={n.id} className="px-4 py-3 border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => { markRead(n.id); if (n.type === 'meeting_request' || n.type === 'meeting') { navigate(`/meetings?id=${n.module_id}&type=${n.type === 'meeting_request' ? 'request' : 'meeting'}`) } else if (n.module_type === 'project' && n.module_id) { navigate(`/projects?projectId=${n.module_id}&tab=queries`) } }}>
...
export default function Layout({ children }) {
  ...
  const searchColors = { lead: '#6D28D9', account: '#10B981', project: '#3B82F6', contact: '#DB2777' }

  const pageTitle = Object.entries(pageTitles).find(([path]) => pathname.startsWith(path))?.[1] || ''

  return (
    <div className="flex h-screen bg-[#F1F5F9]" style={{ fontFamily: "Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: 'antialiased', MozOsxFontSmoothing: 'grayscale' }}>
      {/* Sidebar */}
      <aside style={{
        ...
        <div style={{
          ...
          {!collapsed && <span style={{ fontSize: 16, fontWeight: 800, color: '#0F172A', letterSpacing: '-0.3px' }}>PMS</span>}
        </div>
          ...
          {!collapsed && (
            <>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#0F172A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.full_name || 'User'}</div>
                <div style={{ fontSize: 11, color: '#94A3B8' }}>{user?.role || ''}</div>
              </div>
              <button onClick={logout} style={{ padding: 4, borderRadius: 6, border: 'none', background: '#F1F5F9', color: '#94A3B8', cursor: 'pointer' }} title="Logout">
                <LogOut className="w-3.5 h-3.5" />
              </button>
            </>
          )}
          {collapsed && (
            <button onClick={logout} style={{ padding: 4, borderRadius: 6, border: 'none', background: 'transparent', color: '#94A3B8', cursor: 'pointer' }} title="Logout">
              <LogOut className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={() => setCollapsed(!collapsed)} title={collapsed ? 'Show sidebar' : 'Hide sidebar'} style={{
              padding: '6px', borderRadius: 8, border: 'none', background: '#F1F5F9',
              color: '#64748B', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 14, lineHeight: 1,
            }}>
              {collapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}
            </button>
            <span style={{ fontSize: 16, fontWeight: 700, color: '#0F172A' }}>{pageTitle}</span>
            <span style={{ width: 1, height: 16, background: '#E2E8F0' }} />
            <div ref={searchRef} style={{ position: 'relative' }}>
              <Search className="w-3.5 h-3.5" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#94A3B8' }} />
              <input value={searchQuery} onChange={e => doSearch(e.target.value)} placeholder="Search anything..." style={{
                ...
                fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif', background: '#F8FAFC',
              }} />
              {searchOpen && searchResults.length > 0 && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: '#fff', borderRadius: 8, border: '1px solid #E2E8F0', zIndex: 100, maxHeight: 360, overflowY: 'auto', boxShadow: '0 8px 24px rgba(0,0,0,.1)' }}>
                  {searchResults.map((r, i) => (
                    <Link key={`${r.type}-${r.id}`} to={r.url} onClick={() => { setSearchOpen(false); setSearchQuery(''); setSearchResults([]) }}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', textDecoration: 'none', borderBottom: i < searchResults.length - 1 ? '1px solid #F1F5F9' : 'none', transition: 'background .1s' }}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {r.subtitle && <div style={{ fontSize: 11, color: '#94A3B8' }}>{r.subtitle}</div>}
                      </div>
                      {r.label && <span style={{ fontSize: 11, color: searchColors[r.type], fontWeight: 600, flexShrink: 0 }}>{r.label}</span>}
                    </Link>
                  ))}
                </div>
              )}
