...
const FOLDER_ICONS = {
  inbox: Inbox, sentitems: Send, drafts: Edit3, deleteditems: Trash2, archive: Archive,
}
...
export default function EmailInbox() {
  ...
  }, [activeFolder, categoryFilter, statusFilter, priorityFilter, search])
    ...
    try {
      ...
      if (all.length > 0) {
        setActiveFolder(prev => {
          if (prev) return prev
          const inbox = all.find(f => f.well_known_name?.toLowerCase() === 'inbox' || f.name?.toLowerCase() === 'inbox')
          return inbox ? inbox.folder_id : all[0].folder_id
        })
      }
    } catch (e) {}
  }, [])
  ...
  const handleFetch = async () => {
    setLoading(true)
    try { await syncFolders(); await fetchEmails(); loadFolders(); loadMessages(); loadMeta() } catch (e) { setLoading(false) }
  }
          ...
          <div style={{ display: 'flex', gap: 0, flex: 1, minHeight: 0 }}>
            ...
            <div style={{
              ...
              <div style={{ padding: '8px 14px 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: C.secondary, letterSpacing: '0.5px' }}>Folders</span>
                <span style={{ fontSize: 10, color: C.secondary }}>{folders.length}</span>
              </div>
              {folders.length === 0 && accounts.length > 0 && (
                <div style={{ padding: '12px 14px', fontSize: 12, color: C.secondary }}>
                  <button onClick={handleFetch} style={{ background: 'none', border: 'none', color: C.blue, cursor: 'pointer', fontSize: 12, fontFamily: C.font, padding: 0 }}>Sync folders</button>
                </div>
              )}
              {(() => {
                const sorted = [...folders].sort((a, b) => {
                  const aIdx = WELL_KNOWN_ORDER.indexOf(a.well_known_name?.toLowerCase() || '')
                  const bIdx = WELL_KNOWN_ORDER.indexOf(b.well_known_name?.toLowerCase() || '')
                ...
                return sorted.map(f => {
                  const wk = (f.well_known_name || f.name || '').toLowerCase().replace(/\s+/g, '')
                  const FolderIcon = FOLDER_ICONS[wk] || (activeFolder === f.folder_id ? FolderOpen : Folder)
                  const isActive = activeFolder === f.folder_id
                  return (
                    <div key={f.id} onClick={() => { setActiveFolder(f.folder_id); setSelectedMsg(null) }}
                      style={{
                      ...
                      <FolderIcon className="w-3.5 h-3.5" style={{ flexShrink: 0, color: isActive ? C.blue : '#94A3B8' }} />
                      <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.display_name || f.name}</span>
                      {(f.unread_count || 0) > 0 && (
                        <span style={{
                          background: C.blue, color: '#fff', fontSize: 10, fontWeight: 700,
                          padding: '0 6px', borderRadius: 10, lineHeight: '16px', flexShrink: 0,
                        }}>{f.unread_count > 99 ? '99+' : f.unread_count}</span>
                      )}
                    </div>
                  )
                })
            ...
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '0 20px 28px', display: 'flex', gap: 20, flex: 1 }}>
