...
if r.status_code == 200 and r.json().get("requires_otp"):
    ...
    print(f"Got temp_token, verifying OTP 179798...")
    r2 = requests.post(f"{BASE}/api/auth/verify-otp", json={"temp_token": temp_token, "otp_code": "179798"}, verify=False)
    ...
    if r2.status_code == 200:
        ...
        # Test employee access - should be 403
        print("\n\n=== NOW TEST AS EMPLOYEE (should be 403) ===")
else:
    print("Login failed")

# Login as employee
print("=== LOGIN employee ===")
r = requests.post(f"{BASE}/api/auth/login", json={"email": "employee1@test.com", "password": "pass1234"}, verify=False)
print(r.status_code, r.json())
if r.status_code == 200 and r.json().get("requires_otp"):
