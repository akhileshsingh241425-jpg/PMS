...
from models import db, Project, Task, Meeting
...
def overview(current_user):
    ...
    pending_meeting_requests = MeetingRequest.query.filter_by(status='Requested').count()
    pending_meetings = Meeting.query.filter(Meeting.status.in_(['Scheduled', 'Rescheduled'])).count()
    total_pending_meetings = pending_meeting_requests + pending_meetings
    ...
    return jsonify({
        ...
        'pending_meetings': total_pending_meetings,
        'my_tasks': my_tasks,
